...
ConsumerStrategy.prototype.authenticate = function(req) {
  ...
  var consumerKey = params['oauth_consumer_key']
    ...
    , requestorId = params['xoauth_requestor_id']
  ...
  this._consumer(consumerKey, function(err, consumer, consumerSecret) {
    ...
    if (!requestToken) {
      ...
      // ML addition: Require uid (to be verified downstream)
      if (!requestorId) {
        return self.fail(self._challenge('uid_rejected'));
      }

      self._token(requestToken, requestorId, function(err, tokenSecret, info) {
        if (err) { return self.error(err); }
        if (!tokenSecret) { return self.fail(self._challenge('token_rejected')); }

        validate(info, tokenSecret, function() {
          info = info || {};
          info.scheme = 'OAuth';
    ...
    function validate(info, tokenSecret, ok) {
      ...
      if (self._validate) {
        ...
        // it so desires.
        info = info || {user: undefined};
        self._validate(info.user, timestamp, nonce, function(err, valid) {
          if (err) { return self.error(err); }
...
ConsumerStrategy.prototype._challenge = function(problem, advice) {
  ...
  console.log('Auth failure: ' + challenge);
  return challenge;
}
