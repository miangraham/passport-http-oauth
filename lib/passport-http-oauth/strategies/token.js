...
TokenStrategy.prototype.authenticate = function(req) {
  ...
  this._consumer(consumerKey, function(err, consumer, consumerSecret) {
    ...
    if (!requestorId) { return self.fail(self._challenge('uid_rejected')); }

    self._verify(accessToken, requestorId, verified);
    ...
    function verified(err, user, tokenSecret, info) {
      ...
      if (self._validate) {
        ...
        // it so desires.
        self._validate(user, timestamp, nonce, function(err, valid) {
          if (err) { return self.error(err); }
